  ...
  return (
    <div className="flex items-center gap-2">
      <button onClick={applyTemplate} disabled={busy}
        className="inline-flex items-center gap-2 px-3 py-1.5 border rounded-lg bg-white hover:bg-gray-50" title="Apply first template">
        <CalendarPlus size={16}/> {busy ? 'Applying…' : 'Apply Template'}
      </button>
      <button onClick={copyLastWeek} disabled={busy}
        className="inline-flex items-center gap-2 px-3 py-1.5 border rounded-lg bg-white hover:bg-gray-50" title="Copy last week">
